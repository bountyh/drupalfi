...
function drupalfi_preprocess_html(&$vars) {
  ...
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . drupalfi_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = drupalfi_id_safe('page-'. $path);
    $vars['classes_array'][] = drupalfi_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('drupalfi_ie_enabled')) {
    $drupalfi_ie_enabled_versions = theme_get_setting('drupalfi_ie_enabled_versions');
    if (in_array('ie8', $drupalfi_ie_enabled_versions, TRUE)) {
...
function drupalfi_preprocess_page(&$vars, $hook) {
...
function drupalfi_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from drupalfi_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function drupalfi_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function drupalfi_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('drupalfi_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('drupalfi_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('drupalfi_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('drupalfi_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('drupalfi_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function drupalfi_id_safe($string) {
 ...
 *
 */
function drupalfi_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = drupalfi_id_safe($element['#title']);
 ...
 */
function drupalfi_preprocess_menu_local_task(&$variables) {
 ...
 */
function drupalfi_menu_local_tasks(&$variables) {
  $output = '';
